...
old_buildiso=$(grep -oP "buildraVersion='v\d{2}\.\d{2}\.\d{2}'" build-scripts/build-the-iso.sh | grep -oP 'v\d{2}\.\d{2}\.\d{2}' | head -1)
...
sed -i "s|\(.*buildraVersion='\)[^']*\('.*\)|\1$newversion\2|" build-scripts/build-the-iso.sh
